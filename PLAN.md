...
> **版本：** v1.2  
> **创建日期：** 2026-05-23 | **最后更新：** 2026-05-28  
> **作者：** STOA Team  
> **状态：** 进行中
...
| 地址 | 内容 | 状态 |
|------|------|------|
| https://app.stoaedu.ch | React SPA 前端 | ✅ 可访问 |
| https://api.stoaedu.ch/health | FastAPI 后端 API | ✅ `{"status":"ok"}` |

### Sprint 完成度

| Sprint | 目标 | 基础设施 | 后端 API | 前端 UI | 前后端联调 | 测试 |
|--------|------|---------|---------|---------|-----------|------|
| **Sprint 1** | 认证系统 | ✅ | ✅ | ✅ | ❌ 未联调 | ❌ |
| **Sprint 2** | AI 答疑 | ✅ | ✅ | ✅ | ❌ 未联调 | ❌ |
| **Sprint 3** | 老师接管 + 家长面板 | ✅ | ✅ | ✅ | ❌ 未联调 | ❌ |
| **Sprint 4** | 周报 + 上线准备 | 部分 | 部分 | 部分 | ❌ | ❌ |

### 超计划完成（原 Sprint 4 任务）

| 任务 | 完成情况 |
|------|---------|
| 自定义域名（app/api.stoaedu.ch） | ✅ 已完成 |
| *.stoaedu.ch 通配符 ACM 证书 | ✅ 已完成 |
| GitHub Actions CI/CD（三仓库） | ✅ 已完成，git push 自动部署 |
| Route 53 DNS A/ALIAS 记录 | ✅ 已完成 |

### 🚧 当前最大缺口

1. **前后端未联调** — 前端页面已有但 API 调用仍用 demo 数据，Cognito 登录流程未验证
2. **后端 Sprint 4 功能** — EventBridge 定时任务、`report_service.py`（周报生成）、SES 邮件发送 未实现
3. **单元/集成/E2E 测试** — 零测试覆盖
4. **测试用户** — Cognito 中无测试账号，无法端到端验证登录

### ⏭ 立即下一步

→ **前后端联调（Sprint Bridge）**：创建 Cognito 测试用户，打通登录 → 提问 → AI 解答完整链路
