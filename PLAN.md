...
| Region | eu-central-2（苏黎世） | FrontendStack 在 us-east-1 |
...
| 任务 | 优先级 | 预估工时 | 负责人 |
|------|--------|----------|--------|
| CDK Bootstrap（eu-central-2 + us-east-1） | P0 | 2h | 后端 |
...
- [ ] CDK bootstrap 完成（eu-central-2 + us-east-1）
...
| AWS Bedrock eu-central-2 不支持 Claude Haiku | 中 | 高 | 备用 Region us-east-1 跨区调用；或切换 Amazon Nova Lite |
